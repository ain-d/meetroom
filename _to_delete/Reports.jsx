...
function Reports() {
  ...
  const [stats, setStats] = useState({
    ...
    approved: 0,
    cancelled: 0,
    rejected: 0, // ✅ แก้บั๊ก: เดิมไม่มีการดึงสถิตินี้เลย ทำให้ยอดรวมไม่ตรง
    completed: 0,
    no_show: 0,
    checkedIn: 0,
    totalRooms: 0,
  ...
  useEffect(() => {
    const loadStats = async () => {
      ...
      const [
        ...
        approvedRes,
        cancelledRes,
        rejectedRes, // ✅ เพิ่มการดึงสถิติ rejected ที่ขาดไป
        completedRes,
        noShowRes,
        checkedInRes,
        roomsRes,
  ...
  return (
    <main className="page-container">
      <section className="card rpt-wrap">
        ...
        <h2 className="rpt-section-title">ต้องดำเนินการ</h2>
        ...
        <div className="rpt-group rpt-group-resource">
          <div className="rpt-stat-card" style={{ '--accent': 'var(--rpt-gold)' }}>
            ...
            <div className="rpt-stat-body">
              <h3>ห้องประชุม</h3>
              <p className="rpt-stat-number">{stats.totalRooms}</p>
            </div>
          </div>
          <div className="rpt-stat-card" style={{ '--accent': 'var(--rpt-gold)' }}>
            <span className="rpt-stat-icon">👥</span>
            <div className="rpt-stat-body">
              <h3>ผู้ใช้งาน</h3>
              <p className="rpt-stat-number">{stats.totalUsers}</p>
            </div>
          </div>
        </div>
      </section>
...
// สไตล์ทั้งหมดถูก scope ด้วย prefix "rpt-" เพื่อไม่ปะทะกับธีม Dark Forest & Gold ของหน้าอื่น
function ReportsStyles() {
  return (
    <style>{`
      ...
      .rpt-group-featured { grid-template-columns: 1fr; }
