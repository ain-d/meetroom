...
function Calendar() {
  ...
  for (let d = 1; d <= daysInMonth; d++) calendar.push(new Date(selectedDate.getFullYear(), selectedDate.getMonth(), d))
  while (calendar.length % 7 !== 0) calendar.push(null)

  // ✅ แก้ไขตรงนี้: ดึงค่า month และ year เข้ามาใน useMemo แทนการใช้เป็น Dependencies
          ...
          <>
            <div className="cal-toolbar">
              <div className="cal-nav">
                <button onClick={() => setSelectedDate(new Date(selectedDate.getFullYear(), selectedDate.getMonth() - 1, 1))} className="cal-btn-icon">◀</button>
                <button onClick={() => setSelectedDate(new Date())} className="cal-btn-today">วันนี้</button>
                <button onClick={() => setSelectedDate(new Date(selectedDate.getFullYear(), selectedDate.getMonth() + 1, 1))} className="cal-btn-icon">▶</button>
              </div>
              <h2 className="cal-title">{monthNames[selectedDate.getMonth()]} {selectedDate.getFullYear() + 543}</h2>
            ...
            {viewMode === 'month' && (
              <><div className="cal-weekdays">{dayNames.map((day) => <div key={day} className="cal-weekday">{day}</div>)}</div>
              <div className="cal-grid">{calendar.map((date, index) => { if (!date) return <div key={index} className="cal-cell empty" />; const isToday = date.toDateString() === new Date().toDateString(); const isSelected = date.toDateString() === selectedDate.toDateString(); const count = bookingCountsMap.get(date.getDate()) || 0; return (<button key={index} className={`cal-cell ${isSelected ? 'is-selected' : ''} ${isToday ? 'is-today' : ''}`} onClick={() => { setSelectedDate(date); setViewMode('day') }}><span className="cal-num">{date.getDate()}</span>{count > 0 && <span className="cal-badge">{count}</span>}</button>) })}</div></>
            )}
            {viewMode === 'day' && (
