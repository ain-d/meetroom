...
function RoomsAdmin() {
  ...
  // ✅ ย้ายมาไว้ด้านบนเพื่อแก้ ESLint Error
  const refreshRooms = async () => {
    try {
      const { data, error } = await supabase.from('rooms').select('id, name, prefix, capacity, is_active, image_url, facilities, room_status(status)').eq('is_active', true).order('name')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (imageFile) { const fileName = `${Date.now()}-${imageFile.name}`; const { error: uploadError, data: uploadData } = await supabase.storage.from('room-images').upload(fileName, imageFile); if (uploadError) { setMessage({ type: 'error', text: `อัปโหลดรูปไม่สำเร็จ: ${uploadError.message}` }); return } imageUrl = supabase.storage.from('room-images').getPublicUrl(uploadData.path).data.publicUrl }
      const roomPayload = { name: form.name, capacity: Number(form.capacity), is_active: true, image_url: imageUrl, facilities: facilities, prefix: form.prefix.toUpperCase().trim() || 'BK' }
      ...
      setForm({ name: '', capacity: '', status: '', prefix: '' }); setEditingRoom(null); setImageFile(null); setFacilities([]); await refreshRooms()
    } catch (error) { setMessage({ type: 'error', text: 'เกิดข้อผิดพลาดในการบันทึกข้อมูล กรุณาลองใหม่' }) } finally { setLoading(false) }
  }

  const handleEdit = (room) => { setEditingRoom(room); setForm({ name: room.name, capacity: room.capacity.toString(), status: room.room_status?.status || '', prefix: room.prefix || '' }); setFacilities(room.facilities || []); setMessage({ type: '', text: '' }) }
  ...
  const handleCancelEdit = () => { setEditingRoom(null); setImageFile(null); setForm({ name: '', capacity: '', status: '', prefix: '' }); setFacilities([]); setMessage({ type: '', text: '' }) }
  ...
  return (
    <main className="page-container">
      <section className="card">
        <div className="page-header"><h1>🏢 จัดการห้องประชุม</h1><p>เพิ่ม แก้ไข หรือปิดการใช้งานห้องประชุม</p></div>
        ...
        <form className="register-form" onSubmit={handleSubmit}>
          ...
          <label><span>ความจุ (จำนวนคน)</span><input type="number" name="capacity" value={form.capacity} onChange={handleChange} placeholder="10" /></label>
        ...
        <div className="room-table-wrapper">
          ...
          {loading ? <p className="text-center">กำลังโหลด...</p> : (
            <table className="room-table">
              ...
              <tbody>{rooms.map((room) => (<tr key={room.id}><td>{room.image_url ? <img src={room.image_url} alt={room.name} /> : <div className="img-placeholder">ไม่มีรูป</div>}</td><td><strong>{room.name}</strong></td><td>{room.capacity} คน</td><td>{room.facilities && room.facilities.length > 0 ? (<div className="facilities-list">{room.facilities.map((fac, index) => <span key={index} className="facility-badge">{fac}</span>)}</div>) : <span className="text-empty">ไม่มีข้อมูล</span>}</td><td><span className={`status ${room.room_status?.status}`}>{getStatusLabel(room.room_status?.status)}</span></td><td><div className="table-actions"><button onClick={() => handleEdit(room)}>แก้ไข</button><button className="danger-button" onClick={() => handleDelete(room)}>ปิดใช้งาน</button></div></td></tr>))}</tbody>
            </table>
          )}
        </div>
      </section>
